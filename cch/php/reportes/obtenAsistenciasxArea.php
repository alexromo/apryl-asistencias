...
if (!isset($_REQUEST['area']) || ($_REQUEST['area'] == 0) ||
...
else
    {
        ...
        if (!$rs->fields) $table = '<div class="error">No hay Registros</div>';
                ...
                $js = '<input type="button" value="Muestra/Oculta Grafica " onclick="DrawPieChart('.$asistencias.','.$faltas.');$(\'graphcontainer\').toggle();" />';
                ...
                if (!$rs->fields) $table2 = '<div class="error">No hay Faltas Registradas</div>';
                        ...
                        $table2 = '<table width="95%" cellspacing="1" >'
                                 .'<thead><tr><th colspan="5">FALTAS</th></tr><tr><th>id</th><th>Materia</th><th>Asistencias</th></thead>'
                                 .'<tbody>';
                        foreach ($rs as $k => $row)
                            {
                                $table2 .= "<tr><td>".$row[0]."</td><td>".$row[1]."</td><td>".$row[2]."</td>";
                            }
